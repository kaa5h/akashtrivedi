...
/* Renders **bold** and [text](url) inside a paragraph. */
function BT({t}){
  return t.split(/(\*\*[^*]+\*\*|\[[^\]]+\]\([^)]+\))/g).map((s,i)=>{
    if(/^\*\*[^*]+\*\*$/.test(s))return<strong key={i} className="bpb">{s.slice(2,-2)}</strong>;
    const m=s.match(/^\[([^\]]+)\]\(([^)]+)\)$/);
    if(m)return<a key={i} className="bpl" href={m[2]} target="_blank" rel="noopener noreferrer">{m[1]}</a>;
    return s;
  });
}
/* Blog body: blank-line separated blocks. "## " = section heading,
   "---" = divider, *wrapped in single asterisks* = small italic note. */
function BlogBody({bd}){
  return<div className="bpbd">{bd.split("\n\n").map((b,i)=>{
    if(b.trim()==="---")return<div key={i} className="bphr"/>;
    if(b.startsWith("## "))return<h2 key={i} className="bpsh">{b.slice(3)}</h2>;
    if(/^\*[^*][\s\S]*\*$/.test(b))return<p key={i} className="bpci"><BT t={b.slice(1,-1)}/></p>;
    return<p key={i} className="bppr"><BT t={b}/></p>;
  })}</div>;
}
...
export default function Page({initial}){
  ...
  useEffect(()=>{setM(true)},[]);
  const nav=(t,p=null,b=null,skipPush=false)=>{setFd(true);setTimeout(()=>{setPg(t);setProj(p);setBp(b);window.scrollTo(0,0);if(!skipPush)window.history.pushState({pg:t,proj:p?.id||null,bp:b?.id||null},"",pathFor(t,b));setTimeout(()=>setFd(false),50)},280)};
  useEffect(()=>{window.history.replaceState({pg:pg,proj:null,bp:bp?.id||null},"",pathFor(pg,bp));const onPop=(e)=>{const s=e.state;if(s){const p=s.proj?P.find(x=>x.id===s.proj):null;const b=s.bp?BL.find(x=>x.id===s.bp):null;nav(s.pg,p,b,true)}else{const v=viewFor(window.location.pathname.split("/"));nav(v?.pg||"home",null,v?.bp||null,true)}};window.addEventListener("popstate",onPop);return()=>window.removeEventListener("popstate",onPop)},[]);
...
.r{min-height:100vh;background:var(--bg);color:var(--fg);font-family:'Inter',ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,'Noto Sans',sans-serif;font-optical-sizing:auto;font-variant-numeric:slashed-zero;overflow-x:hidden;overflow-x:clip}
...
.cvp{max-width:720px;margin-left:auto;margin-right:auto;padding-top:clamp(16px,3.2vw,32px);padding-bottom:80px;min-height:100vh}.cvh{display:grid;grid-template-columns:1fr auto;gap:32px;align-items:start;margin-bottom:clamp(36px,5vw,56px)}.cvt{font-weight:650;font-size:clamp(32px,5vw,48px);color:var(--fg);letter-spacing:-.03em;margin-bottom:4px}.cvst{font-size:15px;color:var(--f2)}
...
.blp{max-width:720px;margin-left:auto;margin-right:auto;padding-top:clamp(16px,3.2vw,32px);padding-bottom:80px;min-height:100vh}.blt{font-weight:650;font-size:clamp(32px,5vw,48px);color:var(--fg);letter-spacing:-.03em;margin-bottom:12px}.bls{font-size:15px;color:var(--f2);line-height:1.6;max-width:560px;margin-bottom:clamp(36px,5vw,56px)}
